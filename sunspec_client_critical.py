...
'''
Float values need to be converted to an equivalent integer*scaling_factor formi and
converted integer must be within the bounds of it's datatype.
'''
...
'''
Data to be written to the SunSpec device needs to be packed into bytes
'''
...
'''
Picks the latest value in the write queue and writes it to the SunSpec device.
Flushes the rest of the queue.
'''
...
'''
Critical data read from the SunSpec device is published to a topic on the MQTT broker.
'''
...
'''
Continously polls the SunSpec device for critical data
'''
